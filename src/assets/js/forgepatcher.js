...
class ForgePatcher {
  ...
  getProcessorMainClass(jarPath) {
    ...
    if (!jarManifest)
      throw new Error(
        "Unable to find the JAR manifest. Is the archive corrupted?",
      );

    const mainClass = jarManifest
      .toString()
      .replaceAll("\r", "")
      .split("\n")
    ...
    if (!mainClass)
      throw new Error(
        "Unable to find the main class in the jar manifest. Is the archive corrupted?",
      );
  ...
  async patch() {
    ...
    for (const processor of processors) {
      ...
      let javaBin = ConfigManager.getJavaExecutable(
        this.serverModule.rawServer.id,
      );
      if (javaBin.endsWith("javaw"))
        javaBin = javaBin.replace("/javaw", "/java");
      else if (javaBin.endsWith("javaw.exe"))
        javaBin = javaBin.replace("\\javaw.exe", "\\java.exe");
      ...
      const mainClass = this.getProcessorMainClass(
        join(libDir, MavenUtil.mavenIdentifierAsPath(processor.jar)),
      );
