...
fn entry_for(uuid: &str) -> Result<Entry, AuthError> {
    Entry::new(SERVICE_NAME, uuid).map_err(|e|
        AuthError::Keyring(format!("access failed for {uuid}: {e}"))
    )
}

/// Store (or overwrite) the token set for an account.
#[tauri::command]
pub async fn store_account_tokens(uuid: String, tokens: AccountTokens) -> Result<(), AuthError> {
    let entry = entry_for(&uuid)?;

    let serialized = serde_json
        ::to_string(&tokens)
        .map_err(|e| AuthError::Other(format!("Failed to serialize tokens for {uuid}: {e}")))?;

    entry
        .set_password(&serialized)
        .map_err(|e| AuthError::Keyring(format!("store failed for {uuid}: {e}")))?;
...
#[tauri::command]
pub async fn get_account_tokens(uuid: String) -> Result<Option<AccountTokens>, AuthError> {
    let entry = entry_for(&uuid)?;

    match entry.get_password() {
        Ok(serialized) => {
            let tokens: AccountTokens = serde_json
                ::from_str(&serialized)
                .map_err(|e| {
                    AuthError::Other(format!("Failed to parse stored tokens for {uuid}: {e}"))
                })?;
            Ok(Some(tokens))
        }
        Err(keyring::Error::NoEntry) => Ok(None),
        Err(e) => Err(AuthError::Keyring(format!("read failed for {uuid}: {e}"))),
    }
}
...
#[tauri::command]
pub async fn remove_account_tokens(uuid: String) -> Result<(), AuthError> {
    let entry = entry_for(&uuid)?;

    match entry.delete_credential() {
        Ok(()) | Err(keyring::Error::NoEntry) => Ok(()),
        Err(e) => Err(AuthError::Keyring(format!("remove failed for {uuid}: {e}"))),
    }
}
